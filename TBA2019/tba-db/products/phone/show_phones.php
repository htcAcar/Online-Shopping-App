...
//Check for mandatory parameters
	...
	$query = "SELECT product_ID, name, color, screen_size, brand, memory, camera, price FROM phone ";
	//Prepare the query
	if($stmt = $con->prepare($query)){
	$stmt->execute();
	//Bind the fetched data to $movieId and $movieName
	$stmt->bind_result($product_ID, $name, $color, $screen_size, $brand, $memory, $camera, $price);
	...
	while($stmt->fetch()){
		...
		$userArray["memory"] = $memory;
		$userArray["camera"] = $camera;
